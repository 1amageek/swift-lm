    ...
    private func format(_ entry: DispatchEntry) -> String {
        ...
        if let projection = entry.fragment as? ProjectionDescribing {
            ...
            if fields.count == 1, let f = fields.first {
                ...
                let names = fields.map(\.field).joined(separator: ",")
                kind = "batchedProjection(\(fields.count)-way: \(names))"
            }
        } else {
    ...
    private func projectionComponents(for entry: DispatchEntry) -> [ProjectionComponent]? {
        ...
        if let linear = fragment as? LinearFragment {
            return [
                ProjectionComponent(
                    role: linear.field,
                    tensorName: entry.parameterBindings.first(where: { $0.role == linear.field })?.tensorName,
