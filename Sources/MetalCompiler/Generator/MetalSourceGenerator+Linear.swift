...
// MARK: - Linear Kernels
    ...
    public static func generateRecurrentBlockPartialProjection(
        name: String,
        bufferPrecision: BufferPrecision,
        weightFormat: WeightFormat,
        tileElements: Int = 256
        ...
        kernel void \(name)(
            ...
            constant uint& partialRowStride              [[buffer(8)]],
            ...
            const uint safeRow = row < outputDimension ? row : 0u;
            device const \(wt)* weightRow = weight + safeRow * inputDimension + groupInputBase;
    ...
    public static func generateRecurrentBlockGroupOwnedPartialProjection(
        name: String,
        bufferPrecision: BufferPrecision,
        weightFormat: WeightFormat,
        tileElements: Int = 256
        ...
        kernel void \(name)(
            ...
            const uint seqPos = gid.y;
            if (group >= groupCount || seqPos >= sequenceLength) {
                return;
            }
            ...
            for (uint rowBase = 0; rowBase < outputDimension; rowBase += rowsPerThreadgroup) {
    ...
    public static func generateVocabGEMVBlocked8x128(
        name: String,
        bufferPrecision: BufferPrecision,
        weightFormat: WeightFormat
    ) -> String {
        guard weightFormat.isBFloat16 else {
            return generateVocabGEMV(
                name: name,
                bufferPrecision: bufferPrecision,
                weightFormat: weightFormat
            )
        ...
        kernel void \(name)(
            ...
            if (active) {
                ...
                device const ushort4* weightLane = (device const ushort4*)weight
    ...
    public static func generateVocabGEMVBlocked8x128PartialArgmax(
        name: String,
        bufferPrecision: BufferPrecision,
        weightFormat: WeightFormat
    ) -> String {
        guard weightFormat.isBFloat16 else {
            return generateVocabGEMVPartialArgmax(
                name: name,
                bufferPrecision: bufferPrecision,
                weightFormat: weightFormat
            )
