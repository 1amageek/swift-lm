...
- HuggingFace reference dump は実行 dtype を保持する。BF16 モデルの参照値を `.half()` などで FP16 に変換して保存してはならない。
- prefill を再実行する probe は実行状態を変える可能性がある。合否判定には、同じ decode 実行中の live buffer を blit/staging で読む非摂動 probe を使う。
- private/shared Metal buffer の直接 pointer read を診断の前提にしない。GPU が書いた scratch / KV / hidden は blit 後に staging buffer から読む。
- config 由来の数値を hardcode しない。LFM2 の Q/K norm epsilon は `config.norm_eps` と一致させる。
...
- thinking model が 256 greedy tokens 以内に `</think>` を閉じず、final answer が空
- HuggingFace greedy prefix と Metal greedy prefix が argmax divergence を起こしている
- local path dependency、診断用 precision override、常時 debug log が残っている
...
- token prefix parity は release gate として扱う。top-2 が近い argmax 反転でも、free-running 出力が別経路に入るなら許容しない。
- final logits だけで原因を決めない。`q_norm` / `k_norm` / RoPE 後 K / V / attention pre-o_proj / layer residual / final norm / output head を分けて比較する。
- probe が通った場合でも、probe 無効化で同じ prefix parity が通ることを確認する。probe による warmup や prefill 再実行で失敗が隠れることがある。
