...
Use the script below to validate the current SSM artifact set:

```bash
scripts/benchmarks/run-ssm-artifact-validation.sh
```

Each run writes a timestamped result bundle under
`.test-artifacts/ssm-artifact-validation/<timestamp>/`. The bundle contains a
`summary.csv` with `phase`, `filter`, `environment`, `result`, and `log`
columns, plus one log file per focused gate. A failed gate records a `fail` row
before exiting, so handoff review can inspect the exact failing command output
without relying on terminal scrollback.

To regenerate the artifacts first, then run the reconstruction gates:

```bash
scripts/benchmarks/run-ssm-artifact-validation.sh --generate
```
...
SWIFTLM_VALIDATE_QWEN_ROUTE_READINESS_ARTIFACTS=1 \
  swift test -Xswiftc -DENABLE_METAL_PROBES \
  --filter Qwen35PrefillProfileTests/currentRouteReadinessArtifactsCanBeReconstructedWhenRequested
```

This gate is intentionally not enabled by default because `.test-artifacts`
are generated by separate heavy probes. When enabled, it requires all three CSV
inputs to exist, reconstructs `qwen35-prefill-route-readiness.csv`, and fails
if route-promotion roles are stale aggregate buckets instead of the exact
full-profile route keys.
