...
| Sparse MoE parallel router | done | Router logits now run across the expert grid with separate score/select kernels, cutting decode profile time from `26.3ms` to `13.2ms` per profiled token |
...
| Decode timing breakdown | `perl -e 'alarm shift; exec @ARGV' 120 swift test --filter 'LFM25A1BRealBundleTests/defaultSparseMoERouteReportsDecodeTimingBreakdown'` | pass; HF 64-token trace matches exactly; default route uses `sparse_moe_bf16_gate_up_packed4` and `sparse_moe_bf16_down_packed4`; measured decode-only `0.747s` wall / `0.711s` GPU, `84.3` wall tok/s / `88.6` GPU tok/s |
...
| Decode profile | `perl -e 'alarm shift; exec @ARGV' 120 swift test --filter 'LFM25A1BDecodeProfileTests'` | pass; per-token profiled GPU time `11.0ms`; largest families are `gate_up_packed4` `32.2%`, `down_packed4` `15.9%`, vocab GEMV `12.6%`, and router score/select combined `7.3%` |
...
| Decode-only timing reaches `84.3` wall tok/s and `88.6` GPU tok/s | The 80 tok/s decode-only target is cleared on the exact HF 64-token trace through default packed4 projection kernels; disabling packed4 drops the same gate to `69.2` wall tok/s and `72.7` GPU tok/s | Keep packed4 as the default BF16 MoE route and retain `SWIFTLM_SPARSE_MOE_DISABLE_PACKED4=1` for A/B diagnostics |
| Decode profile changed from router-bound to expert-projection-bound | Router share dropped from `53.5%` to `7.3%`; packed `gate_up` and `down` now account for `48.1%` combined | Next speed lever is larger expert-token batching or projection fusion, not scalar BF16 loop tuning |
